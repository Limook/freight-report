...
function renderTripsList() {
  ...
  if (statsContainer) {
    statsContainer.innerHTML = `
      <div style="flex: 1; background-color: var(--bg-card); border: 1px solid var(--bg-card-border); padding: 10px 14px; border-radius: var(--radius-sm); display: flex; align-items: center; justify-content: space-between;">
...
function updateTrackerUI() {
  ...
  if (optionActions && mainActions) {
    ...
    if (step === 0) {
      optionsHtml = `
        <button class="btn-tracker-option" onclick="startTrackerWithDeparture(true)">
      ...
      optionsHtml = `
        ${resetBtnHtml}
        <button class="btn-tracker-option" onclick="${isViaDisabled ? '' : 'addTrackerWaypoint()'}" ${isViaDisabled ? 'disabled' : ''}>
      ...
      optionsHtml = `
        ${resetBtnHtml}
        <button class="btn-tracker-option" onclick="completeTrackerAndRegister(true)">
